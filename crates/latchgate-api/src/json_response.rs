...
impl JsonResponse {
    ...
    /// HTTP status code. Useful for assertions in tests.
    #[allow(dead_code)]
...
mod tests {
    ...
    #[tokio::test]
    async fn detail_format() {
        let resp = JsonResponse::new(StatusCode::BAD_REQUEST, "invalid_input")
            .field("detail", "too long")
            .into_response();
        assert_eq!(resp.status(), StatusCode::BAD_REQUEST);
        assert_eq!(
            body_string(resp).await,
            r#"{"error":"invalid_input","detail":"too long"}"#,
        );
    }
    ...
    async fn status_accessor() {
        let jr = JsonResponse::new(StatusCode::NOT_FOUND, "not_found");
        assert_eq!(jr.status(), StatusCode::NOT_FOUND);
    }
}
